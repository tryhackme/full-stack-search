...
function App() {
  ...
  const fetchData = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const hotelsData = await fetchAndFilterHotels();
    const filteredHotels = hotelsData.filter(({ hotel_name }) =>
      hotel_name.toLowerCase().includes(event.target.value.toLowerCase())
    );
